...
class WiziappMediaExtractor{
			...
			if ($element_key != 'text'){
				...
				*/
				if (strtolower($element_key) == 'img'){
					WiziappProfiler::getInstance()->write("Handling IMG", "WiziappMediaExtractor._process");
					$processed = $this->_handleImage($element_val, $element);
					WiziappProfiler::getInstance()->write("Done Handling IMG", "WiziappMediaExtractor._process");
					...
					*/
					WiziappProfiler::getInstance()->write("Handling link", "WiziappMediaExtractor._process");
					...
					if ( $pos !== FALSE ){
						...
						if ( $type == 'audio' ){
							WiziappProfiler::getInstance()->write("Handling audio link", "WiziappMediaExtractor._process");
							$processed = $this->_handleAudio($element_val['attributes']['href'], $element_val, $element);
							WiziappProfiler::getInstance()->write("Done Handling audio link", "WiziappMediaExtractor._process");
						}
					}
	...
	function _handleVimeoVideo($src){
		...
		WiziappLog::getInstance()->write('DEBUG', ">>> Vimeo API::" . print_r($vimeo_api, TRUE),
			'_handleVimeoVideo');

		$r = new WiziappHTTPRequest();
		$response = $r->external(array(), $vimeo_api, 'GET', array());
			...
			if ($vimeo_movies){
				...
				if (!empty($movie['title'])){
					$obj = array(
						...
						'actionURL' => WiziappLinks::videoLink('vimeo', $vimeo_id, "http://vimeo.com/m/#/{$clipId}"),
					);
				}
			}
		}
		...
		if (!is_wp_error($response)) {
			...
			if ($blipResponse){
				...
				$obj = array(
					...
					'actionURL' => WiziappLinks::videoLink('blip.tv', $blipResponse['itemId'], $blipResponse['url']),
				);
			}
		}
		return $obj;
	...
	*/
	function _handleFlashObjects($element, $force_src = '', $full_element=array()){
		...
		WiziappLog::getInstance()->write('DEBUG', ">>> The src is {$src}", '_handleFlashObjects');
		if (empty($src)){
	...
	function _handleAudio($src, $element, $full_element, $just_return = false) {
		...
		if ($element) {
			foreach($element['childs'] as $child){
				WiziappLog::getInstance()->write('DEBUG', "checking child... : " . print_r($child, TRUE), 'media_extractor.WiziappMediaExtractor._handleAudio');
				// @todo add the thumb;
			}
		}
		...
		$obj = array(
			...
			'actionURL' => WiziappLinks::audioLink('audio', $src),
			'imageURL' => $image,
		);
