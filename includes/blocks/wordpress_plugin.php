	...
	if ($data !== FALSE) {
		foreach($data as $content_id => $medias) {
			...
			if (count($medias) >= $minimumForAppearInAlbums) {
				...
				WiziappLog::getInstance()->write('DEBUG', "The Wordpress album ID is " . $content_id,
...
function wiziapp_get_wordpress_album($images = array(), $postId = 0, $albumId = 0) {
	...
	if ($data !== FALSE) {
		WiziappLog::getInstance()->write('DEBUG', "The Wordpress album is " . print_r($data[$postId], TRUE),
