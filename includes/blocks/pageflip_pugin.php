...
function wiziapp_get_pageflip_album($images_external = array(), $albumId = 0, $postId = 0){
	global $pageFlip;
	WiziappLog::getInstance()->write('DEBUG', "Got a request for a pageflip album: " . print_r($albumId, TRUE), "wiziapp_get_pageflip_album");
	if(is_object($pageFlip) && is_a($pageFlip, 'pageFlip_plugin_base')){
		WiziappLog::getInstance()->write('DEBUG', "The album id is: {$albumId}", "wiziapp_get_pageflip_album");
		if(empty($albumId)) {
...
//            }
	...
	WiziappLog::getInstance()->write('DEBUG', "About to return the images: " . print_r($images, TRUE),
